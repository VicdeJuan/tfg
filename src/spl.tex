...
So the state of the program when we passed the first line is:

\[ \pc = 2 \andcond x=-2\]

This formula give all the information we have of the execution state. We know the program counter and the states and values of all variables.
...
\[
\pc = 2 \andcond x=-2 \pc'=3 \andcond x'=x \andcond x' < 0
\]
...
\[
\pc = 3 \andcond x=-2 \pc'=4 \andcond x'= -x 
\]

This steps we have illustrate are, actually, transition relations.
...
With this simple example, we have shown what a program counter and what a transition relation are. Now we can define the possible instructions a program may have. We have just took the set of instructions the programs we are going to proof have.
