...
\subsubsection{Common}
	...
	\begin{axiomdescription}[next-error--is--null]
		...
		\explanation{ The field \fNext of the \fError \cell is \fNull address.
		}
		\begin{formula}
			next(error) = null
		\end{formula}
...
\subsubsection{Arithmetic}
	...
	\begin{axiomdescription}[nums-are-different]
		...
		\explanation{ All numbers are different one another. \footnote{This axiom is necessary because \spass only includes the equality theory. Thus, 0,1,...,55 are 0-ary functions that could be equal.}
		}
		\begin{formula}
...
\subsubsection{Set theory}
	...
	This set of axioms model the set theory. As there are 3 types of set, this subset of axioms are replicated for $\Sigma_{\sSetAddr},\Sigma_{\sSetElem}$ and $\Sigma_{\sSetTid}$.
	
	In table \ref{table:axiom_set} is gathered the uses of each group of 3 axioms in each invariant.
...
This subset of axioms and the following are the specific axioms needed for this thesis.
%
This are quite more complex than the previous ones because they have the substance of the theories.
	...
	\begin{axiomdescription}[nextreg]
		...
		\explanation{
			...
			%
			If $a$ is not \fNull, then the node pointed by $a$ is also reachable from $b$.
		}
		\begin{formula}
...
\subsubsection{Important}
	...
	\begin{axiomdescription}[lock-keeps-heap--data \& lock-keeps-heap--next]
		...
		\explanation{
			In the same way a \fLock statement preserve the \sSetAddr reachable from any \addr $hd$, the same states for some other functions apart from \fAddrToSet.
			%
	...
	\begin{axiomdescription}[not-in-region--not-change-heap-addr]
		...
		\explanation{
			...
			%
			Then, modifying a \mem in an \addr which is not reachable from $hp$ preserves the \sSetAddr of reachable \addr from $hp$.
		}
		\begin{formula}
	...
	\begin{axiomdescription}[insert--keeps-addr2set]
		...
		\explanation{
			...
			%
			We call \region to the \fAddrToSet reachable from $hd$.
			%
			...
			\\
			If prev points curr, aux points curr, then making prev point to aux has one unique effect on the \region: aux now is in the \region so every reachable address before the \fUpd is still reachable and aux is reachable too.
		}
		\begin{formula}
	...
	\begin{axiomdescription}[remove--keeps-addr2set]
		...
		\explanation{
			...
			It is needed that the 3 address ($prev,curr,aux$) are different one another and all of them are not null.
			%
			If prev points curr, aux points curr, then making prev point to aux has one unique effect in the \region: curr is now not reachable but every \addr reachable before the \fUpd is still reachable.
		}
		\begin{formula}
